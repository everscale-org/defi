...
class LiquidFTRoot(object):
    ...
    #========================================
    #
    def setKeeperCode(self, msig: SetcodeMultisig, value: int, code: str):
        result = self._callFromMultisig(msig=msig, functionName="setKeeperCode", functionParams={"code":code}, value=value, flags=1)
        return result

    def addKeeper(self, msig: SetcodeMultisig, value: int, depoolAddress: str, depoolFee: int, minimumDeposit: int):
        result = self._callFromMultisig(msig=msig, functionName="addKeeper", functionParams={"depoolAddress":depoolAddress, "depoolFee":depoolFee, "minimumDeposit":minimumDeposit}, value=value, flags=1)
        return result

    def addOrdinaryStake(self, msig: SetcodeMultisig, value: int, depoolAddress: str, notifyAddress: str, body: str):
        result = self._callFromMultisig(msig=msig, functionName="addOrdinaryStake", functionParams={"depoolAddress":depoolAddress, "notifyAddress":notifyAddress, "body":body}, value=value, flags=1)
        return result

    #========================================
    #
    def getRootInfo(self, includeIcon: bool):
        result = self._run(functionName="getRootInfo", functionParams={"includeIcon":includeIcon})
        return result
